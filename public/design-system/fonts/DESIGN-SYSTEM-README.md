...
Both Satoshi and Boska come from a single Fontshare CSS request; JetBrains
Mono from Google Fonts. See `preview/_base.css`:
...
(Satoshi + Boska) and Google Fonts (JetBrains Mono) and place them in this
folder, then swap the CDN imports for `@font-face` blocks. The CSS tokens
in `colors_and_type.css` do not need to change.
