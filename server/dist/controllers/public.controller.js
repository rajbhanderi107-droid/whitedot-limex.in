...
export async function submitSampleRequest(req, res) {
    const companyName = req.body.companyName;
    let companyId;
    if (companyName) {
        const existing = await prisma.company.findFirst({
...
export async function submitCalculatorSubmission(req, res) {
    const submission = await prisma.calculatorSubmission.create({ data: req.body });
    if (req.body.email) {
        await notifyAdmins("Calculator submission", `${req.body.contactPerson || "A visitor"} used the LIMEX calculator.`, "LEAD");
    }
