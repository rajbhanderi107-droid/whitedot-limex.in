...
export function requestLogger(req, res, next) {
    ...
    res.on("finish", () => {
        ...
        if (env.isProduction) {
            ...
            console.log(`[${statusTag}] ${method} ${path} → ${status} (${duration}ms) user=${userId}`);
        }
        else {
            // Verbose in development
            console.log(`[${statusTag}] ${method} ${path} → ${status} (${duration}ms) user=${userId}`, req.query && Object.keys(req.query).length > 0 ? `query=${JSON.stringify(req.query)}` : "");
        }
    });
    next();
}
