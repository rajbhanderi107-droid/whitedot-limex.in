...
export async function listResource(req: Request, res: Response) {
  ...
  if (typeof req.query.kind === "string" && req.query.kind) where.kind = req.query.kind;
  const rows = await (r.def.delegate() as any).findMany({ where, orderBy: { updatedAt: "desc" }, take: 300 });
...
export async function createResource(req: Request, res: Response) {
  ...
  const row = await (r.def.delegate() as any).create({ data: normalizeDates(parsed.data) });
...
export async function updateResource(req: Request, res: Response) {
  ...
  const row = await (r.def.delegate() as any).update({ where: { id }, data: normalizeDates(parsed.data) });
...
export async function deleteResource(req: Request, res: Response) {
  ...
  if (!r) return;
  const id = paramId(req);
  await (r.def.delegate() as any).delete({ where: { id } });
