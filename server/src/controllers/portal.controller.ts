...
export async function getPortalState(_req: Request, res: Response) {
...
export async function updatePortalState(req: Request, res: Response) {
  const state = await prisma.portalState.upsert({
    where: { id: "global" },
...
export async function upsertAutomation(req: Request, res: Response) {
...
export async function listAiAgents(_req: Request, res: Response) {
  return sendSuccess(res, await prisma.aiAgentConfig.findMany());
}

export async function upsertAiAgent(req: Request, res: Response) {
  const id = paramId(req);
...
export async function createAiDraft(req: Request, res: Response) {
  ...
  const { kind, lead } = req.body as { kind: DraftKind; lead: LeadContext };
...
export async function getBiSummary(_req: Request, res: Response) {
  ...
  const topIndustries = industryGroups
    .map((g) => ({ industry: g.industry as string, count: g._count._all }))
...
export async function getDetailedHealth(_req: Request, res: Response) {
  // DB latency
...
export async function getSeoAudit(_req: Request, res: Response) {
  let html = "";
  let status = 0;
  let ms = 0;
  try {
...
export async function getDevopsRuns(_req: Request, res: Response) {
  try {
    ...
    if (!r.ok) return sendError(res, "GITHUB_ERROR", `GitHub API returned ${r.status}.`, 502);
    const json = (await r.json()) as { workflow_runs?: Array<Record<string, unknown>> };
...
export async function getBackupStats(_req: Request, res: Response) {
