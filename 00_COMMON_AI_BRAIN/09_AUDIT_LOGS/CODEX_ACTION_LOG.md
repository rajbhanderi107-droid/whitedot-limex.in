...
## 2026-09-21 — One filter system across all books

Continued from c22947a, whose commit references the user's Claude session. Replaced duplicate product/review chips and the legacy fit/state/trade rail with one shared panel in Route, Visits, Leads and Customers. Added address-based area choices, a mutually exclusive status selector, explicit verification, source and reset. Tools remain separate. Old saved views cannot restore hidden legacy predicates. Existing records, photos, visits and orders are unchanged. Build and TypeScript passed; local browser tests could not start because Chromium is absent, so the updated regression suite will run in GitHub CI before release. Company discovery has not reached 30 per area; no invented entries were added.
