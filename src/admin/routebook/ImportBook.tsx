...
import type { RbImport, RbMark } from "./types.js";
import { rbApi } from "./api.js";
import { getRb, load } from "./store.js";
import { toast } from "./ctx.js";

interface Visit { stopId: string; name: string; time: string; note?: string; outcome?: NonNullable<RbMark["outcome"]>; ticked: boolean; starred: boolean }
...
export function ImportBook({ onClose, onDone }: { onClose: () => void; onDone: () => void }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const marks: RbImport["marks"] = [], events: RbImport["events"] = [];
      for (const r of records) {
        ...
        for (const [kind, value] of [["tick", r.ticked ? "1" : null], ["star", r.starred ? "1" : null], ["note", r.note], ["out", r.outcome]] as const) {
